...
module.exports.loginUser =  function (username, password, options, callback) {
    module.exports.findOneUser(['username', 'email'], username, null,  (err, value) => {
        if(err)
            callback(err)
        else {
            if (bcrypt.compareSync(password, value.password)) {
                ...
                callback({msg: "La comparaison des mots de passe sont fausses.", type_error: "no-comparaison"})
            }
        }
    })
}
...
&& _.filter(tab_field, (e) => { return field_unique.indexOf(e) == -1 }).length == 0 ) {
    ...
    User.findOne({ $or: obj_find }).then((value) => {
        if (value)
            ...
            callback({msg: "Error interne mongo",
        ...
        if (!tab_field || !Array.isArray(tab_field)) {
            msg += "Les champs de recherche sont incorrectes."
        }
        if (!value) {
        ...
        if (_.filter(tab_field, (e) => { return field_unique.indexOf(e) == -1 }).length > 0) {
            var field_no_authorized = _.filter(tab_field, (e) =>{ return field_unique.indexOf(e) == -1})
            msg += msg ? ` Et ($(field_not_authorized.join(',')}) ne sont pas des champs de recherche autorisés.` :
            `Les champs (${field_no_authorized.join(',')}) ne sont pas des champs derecherche autorisés.`
            callback({ msg: msg, type_error: `no-valid`, field_no_authorized: field_no_authorized});
        }
